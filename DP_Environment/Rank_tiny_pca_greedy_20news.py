...
from utils import (
    printargs, dirichlet_partition_indices, surpassed_percentage,
    compute_utilities, to_jsonable_metrics, frobenius_of_lora_AB,
    add_noise_to_lora, measure_noise_ratio_for_client
)
...
ART_DIR = BASE_DIR / "noisy_tiny_news20" / "artifacts"
RESULTS_DIR = BASE_DIR / "noisy_tiny_news20" / "checkpoints"
...
def load_or_save_dataset():
    ...
    if os.path.exists(dataset_file):
        ...
        print("Dataset saved to local path")
...
val_idx = np.random.default_rng(42).choice(
    len(total_global_val_dataset),
    size=int(len(total_global_val_dataset) * 0.1),
    replace=False
)
...
if iid:
    ...
    for i in range(num_clients):
        ...
        client_texts  = global_train_texts[start_idx:end_idx]
    ...
    client_train_indices = dirichlet_partition_indices(
...
for i, (client_texts, _) in enumerate(client_data):
    ...
    cfg = get_client_lora_config(i)
    print(f"Client {i+1} Data: {len(client_texts)} samples, Hash: {text_hash}, LoRA rank={cfg.r}, alpha={cfg.lora_alpha}")
...
if __name__ == "__main__":
    ...
    # sigmas = [0.661 * SIGMA_MAX] * num_clients
    ...
    #                                   eta=0.1, c=1.0, use_ema=True, rho=0.1)
    ...
    for t in range(T):
        ...
        CLIENT_SIGMA = {
            i: (round(float(sigmas[i]), 3), round(float(sigmas[i]), 3))
            for i in range(num_clients)
        }
        ...
        print("[Device Check] torch.cuda.device_count() =", torch.cuda.device_count())
        ...
        gpu_nums = max(1, len(vis_list))
        print(f"\n gpu_nums: {gpu_nums}")

        client_models = [None] * num_clients
        client_classifiers = [None] * num_clients
        client_results = [None] * num_clients

        # 1) first load artifacts
        to_train = []
        for cid, (client_texts, client_labels) in enumerate(client_data):
            ...
            if loaded is not None:
                ...
                client_models[cid] = lora_params
                client_classifiers[cid] = classifier_params
                client_results[cid] = (cid + 1, result)
                print(f"[CACHE] Loaded artifacts for client {cid+1} | rank={client_lora_config.r}")
            else:
                to_train.append((cid, client_texts, client_labels))

        # 2) train missing clients
        if to_train:
            ...
            if RUN_SERIAL:
                for cid, client_texts, client_labels in to_train:
                    ...
                    lora_params, classifier_params, result = train_client(
                        cid, client_texts, client_labels, train_idx, val_idx, gpu_id
                    )
                    ...
                    del lora_params, classifier_params, result
                    gc.collect()
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
            else:
                with ProcessPoolExecutor(max_workers=gpu_nums) as executor:
                    ...
                    for future, cid in futures.items():
                        try:
                            ...
                            print(f"Client {cid + 1} Failed to Return Result: {str(e)}")
        else:
        ...
        if weight_mode == 'rpca':
            ...
            # sigma_hat_B_loo, layer_sigma_dict = noise_by_residual_pca_B_shared_projector_layer(client_models=client_models, b_keys=b_keys, q_subspace=4, q_common=4, layer_stride=1, selected_key=None,  verbose=True)
        ...
        global_accuracy = float(results.get("eval_accuracy", float("nan")))
        ...
        for cid, res in client_results_global_model:
            ...
            print(
                f"Client {cid}: Accuracy={acc:.4f}, Micro F1={micro:.4f}, "
        ...
        if client_models[client_id_for_test] is not None:
        ...
        # ===== each client's metrics after local training =====
        def _as_float(x):
            try:
                return float(x)
            except Exception:
        ...
        del global_model, trainer
        if 'client_trainer' in locals():
            del client_trainer
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        # ===== update sigma for next round =====
        ...
        #         # 注意：这里只加载 base_model 的 state_dict
        ...
        #         # 再单独加载 classifier
        #         client_model.classifier.load_state_dict(client_classifiers[cid], strict=True)
        ...
        #         acc = float(metrics_eval.get("eval_accuracy", float("nan")))
        #         all_acc.append(acc)
